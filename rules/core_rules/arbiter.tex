...
\renewcommand{\yggArbiterText}{%
  ...
  \mysubsection{Splitting Usage Dice}{arbiter-miscellania-splitting-ud}
  ...
  \mysection{Knavery}{arbiter-knave}
   ...
   First - a \KNAVE roll should only be required if the difference between success and failure would be interesting, or when the attempt shouldn't be an automatic success.

  Setting a difficulty for a Knave's \mylink{Whisper}{knave-whispers} abilities means picking a number between 2 and 9. If you set it at 9, it's really only doable by a Knave who's level 8 or 9.  The following chart shows the odds by level for a straight roll (first table) and a roll using common Luck configurations (second table).  
...
\myital{Ed:  This idea is taken from John Carr's \href{https://ageofruins.wordpress.com/2012/10/18/deck-of-many-threats-wilderness-encounters-based-on-playing-cards-plus-a-simple-system-for-provisions/}{Age of Ruins}}
...
\mylist {
  ...
  \mylist {
    ...
    \item[$\square$]  Check if anyone is using any Research, Cunning, or Faith; or attempting to create a Staff, Caduceus, or Magic Sword
  }
